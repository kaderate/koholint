...
`room_labels['178/0']`. Does not refute the booth's existence elsewhere on the island -- joins
`screen3_north_house_door_and_outdoor_telephone_search`'s conclusion that this whole neighborhood
(villager_screen/screen3_north/178-0) has no booth; widen the search radius next time, don't
re-sweep this cluster.
...
- `overworld_screen2` (178/0) is fully surveyed end-to-end (September 16 2026) -- no phone booth,
  no other building, confirmed a single static non-scrolling screen from 2 independent entry points.
  Don't re-search this room for the booth again without a new reason; see
  `world_topology.overworld_screen2_phone_booth_survey`. The search needs a genuinely new area, same
  lesson as the September 16 `screen3_north`/`villager_screen` session before it.
